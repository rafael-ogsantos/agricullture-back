...
export const CreateProducerSchema = Joi.object<ProducerInput>({
    ...
    crops: Joi.array().items(Joi.string().valid(Crops.Coffee, Crops.Corn, Crops.Cotton, Crops.Soy, Crops.Sugarcane)).required(),
});
...
export const UpdateProducerSchema = Joi.object<ProducerInput>({
    ...
    crops: Joi.array().items(Joi.string().valid(Crops.Coffee, Crops.Corn, Crops.Cotton, Crops.Soy, Crops.Sugarcane)),
});